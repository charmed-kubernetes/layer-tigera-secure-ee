...
from charms.reactive import when, when_not, when_any, is_state, set_state, remove_state
...
from charmhelpers.core.host import arch, service, service_restart, service_running
...
def deploy_network_policy_controller():
    ...
    if encoded_creds:
        templates.append(("cnx-pull-secret.yaml", {"credentials": encoded_creds}))

    templates += [
        ("calico-config.yaml", {"etcd_endpoints": etcd.get_connection_string()}),
        (
        ...
        (
            ...
            {"registry": registry, "etcd_cert_hash": etcd_cert_hash},
        ),
        (
        ...
        ("cnx-etcd.yaml", {"registry": registry, "etcd_cert_hash": etcd_cert_hash}),
        ("cnx-policy.yaml", {}),
    ]
    ...
    if hookenv.config("enable-elasticsearch-operator"):
        ...
        templates += [
            ("elasticsearch-operator.yaml", {"registry": registry}),
            (
                "monitor-calico.yaml",
                {"apiserver_ips": json.dumps(apiserver_ips), "registry": registry},
            ),
        ]
...
def pull_calicoctl_image():
    ...
    images = {
        os.path.join(registry, hookenv.config("calico-node-image")): resource_get(
            "calico-node-image"
        ),
        os.path.join(registry, hookenv.config("calicoctl-image")): resource_get(
            "calicoctl-image"
        ),
    }
    ...
    for name, path in images.items():
        if not path or os.path.getsize(path) == 0:
            ...
            if (
                not creds
                or not creds.get("auths")
                or registry not in creds.get("auths")
            ):
                CTL.pull(
                    name,
                )
                ...
                CTL.pull(name, username=username, password=password)
        else:
...
def calicoctl(*args):
    ...
    try:
        run = CTL.run(
            net_host=True,
            mounts={CALICOCTL_PATH: CALICOCTL_PATH, "/tmp": "/tmp"},
            environment={
